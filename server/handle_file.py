...
def handle_file(file, session_id, pinecone_index, tokenizer):
    ...
    return handle_file_string(
        filename, session_id, extracted_text, pinecone_index, tokenizer, file_text_dict
    )
...
def extract_text_from_file(file):
    ...
    elif file.mimetype == "text/x-python":
        # Extract text from python file using ast module
        with open(file, "r") as f:
            tree = ast.parse(f.read())
        extracted_text = ast.get_docstring(tree) or ""
    elif file.mimetype == "text/x-java-source":
        # Extract text from java file using ast module
        with open(file, "r") as f:
            tree = ast.parse(f.read())
        extracted_text = ast.get_docstring(tree) or ""
    else:
...
def handle_file_string(
    filename, session_id, file_body_string, pinecone_index, tokenizer, file_text_dict
):
    """Handle a file string by creating embeddings and upserting them to Pinecone."""
    ...
    for i, (text_chunk, embedding) in enumerate(text_embeddings):
        ...
        vectors.append((id, embedding, {"filename": filename, "file_chunk_index": i}))

        logging.info("[handle_file_string] Text chunk {}: {}".format(i, text_chunk))
    ...
    # Upsert each batch to Pinecone and insert data into the database
    for batch in batches:
        try:
            pinecone_index.upsert(vectors=batch, namespace=session_id)

            logging.info(
                "[handle_file_string] Upserted batch of embeddings for {}".format(
                    filename
                )
            )
            ...
            logging.error(
                "[handle_file_string] Error upserting batch of embeddings to Pinecone: {}".format(
                    e
                )
            )
            raise e
...
def create_embeddings_for_text(text, tokenizer):
    ...
    text_chunks_arrays = [
        text_chunks[i : i + MAX_TEXTS_TO_EMBED_BATCH_SIZE]
        for i in range(0, len(text_chunks), MAX_TEXTS_TO_EMBED_BATCH_SIZE)
    ]
